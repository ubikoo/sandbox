    ...
    const cl_double zhi)
{
    ...
    math::rng::uniform<cl_double> rand;          /* rng sampler */

    std::vector<cl_double4> points(n_points);
    for (auto &it : points) {
        it = cl_double4{
            rand(engine, xlo, xhi),
            rand(engine, ylo, yhi),
    ...
    const cl_double zhi)
{
    ...
    const cl_double4 cell = cl_double4{
        (xhi - xlo) / (cl_double) n_cells,
        (yhi - ylo) / (cl_double) n_cells,
        (zhi - zlo) / (cl_double) n_cells,
        0.0 /*unused*/};
    ...
    std::vector<cl_double4> points;             /* lattice site positions */
    {
        cl_ulong site_ix = 0;
        for (cl_ulong i = 0; i < n_cells; i++) {
            for (cl_ulong j = 0; j < n_cells; j++) {
                for (cl_ulong k = 0; k < n_cells; k++) {
                    ...
                    if (is_valid[site_ix]) {
                        cl_double4 pos{
                            (cl_double) i * cell.s[0],
                            (cl_double) j * cell.s[1],
                            (cl_double) k * cell.s[2]};
    ...
    const cl_double zhi)
{
    ...
    std::array<cl_double4,4> basis{
        cl_double4{            0.0,            0.0,            0.0, 0.0 /*unused*/},
        cl_double4{            0.0, 0.5f*cell.s[1], 0.5f*cell.s[2], 0.0 /*unused*/},
        cl_double4{ 0.5f*cell.s[0],            0.0, 0.5f*cell.s[2], 0.0 /*unused*/},
        cl_double4{ 0.5f*cell.s[0], 0.5f*cell.s[1],            0.0, 0.0 /*unused*/}};
