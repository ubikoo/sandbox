...
Model::Model()
{
     ...
     */
    {
        ...
        core_pragma_omp(parallel default(none) shared(m_rng))
        {
            core_pragma_omp(master)
            {
                m_rng.resize(omp_get_num_threads());
                for (auto &rng : m_rng) { rng.engine.init(); }
            }
        }
    }
...
{
    core_pragma_omp(parallel default(none) shared(m_lattice, m_rng))
    {
        int tid = omp_get_thread_num();

        core_pragma_omp(for schedule(static))
        for (size_t row = 0; row < m_lattice.n_sites(); ++row) {
            flip(row, RED, m_rng[tid]);
        }

        core_pragma_omp(for schedule(static))
        for (size_t row = 0; row < m_lattice.n_sites(); ++row) {
            flip(row, BLACK, m_rng[tid]);
        }
    }
}
...
{
    /*
     * Ensure shift is 0 or 1.
     * If shift is 0, even rows have offset 0 and odd rows have offset 1
     * If shift is 1, even rows have offset 1 and odd rows have offset 0
     */
    shift = shift % 2;
    int32_t offset = (row + shift) % 2;

    /* Try to flip the sites in the specified row. */
    for (uint32_t col = 0; col < m_lattice.n_sites() / 2; col++) {
        int32_t x = row;
        int32_t y = 2 * col + offset;
        ...
        double r = rng.randf(rng.engine, 0.0, 1.0);
        if (r >= exp_e) {
